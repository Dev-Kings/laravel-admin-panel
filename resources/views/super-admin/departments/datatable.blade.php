    ...
    <div class="card-body">
        <div class="col-md-12">
            <div class="form-row inline-flex space-x-6">
                <button type="button" name="create_department" id="create_department" class="mb-4 btn btn-success">Add
                    Department
                </button>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="" method="post" id="sample_form" class="form-horizontal">
                ...
                <div class="modal-body">
                    <h4 align="center" style="margin: 0;">Are you sure you want to delete selected department?</h4>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
    ...
    $(function(){
        $.fn.dataTable.ext.errMode = 'throw';
            var table = $('#data-list').DataTable({
                ...
                ajax: { 
                    "type" : "POST",
                    "url" : "{!! route('super-admin.department.list') !!}",
        ...
        $('#create_form').on('submit', function(event){
            ...
            if($('#action').val() == 'Add'){
                save_url = "{{ route('super-admin.department.store') }}";
            }

            if($('#action').val() == 'Edit'){
                save_url = "{{ route('super-admin.department.update') }}";
            }
        ...
        $(document).on('click', '.edit', function(event){
            ...
            $.ajax({
                url: "/super-admin/department/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                dataType:"json",
                success:function(data){
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url:"/super-admin/department/destroy/"+department_id,
                beforeSend:function(){
